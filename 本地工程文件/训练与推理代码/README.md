...
- `scripts/extract_raw.py`：抽取原始样本图。
- `scripts/per_class_masks.py`：按缺陷类别生成样本可视化。
- `scripts/stitch_samples.py`：拼接样本图，便于报告或 PPT 展示。
...
扩展消融配置还包括训练轮数、学习率、损失权重、增强强度、EfficientNet-B3、SE-ResNeXt50、伪标签等方向。04 号同学补充的更新版结果中，还包含 Strong Augmentation、ResNet50、Loss Dice Heavy 三组更完整的扩展记录。这些配置保留为补充实验材料，不作为报告主结果口径。

自动化辅助脚本：

- `scripts/run_ablation_auto.py`
- `scripts/run_ablation_auto.bat`
- `scripts/run_two_experiments.py`
- `scripts/auto_run_loss.py`
...
额外消融实验的 `config.yaml` / `history.csv` 已整理到：

```text
提交文件/01_课程提交涉及材料/实验结果证据/扩展消融实验/
```

数据分析补充图已整理到：

```text
提交文件/01_课程提交涉及材料/图表素材/数据分析补充图/
```
