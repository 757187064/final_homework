                ...
                if len(lines) > 1:
                    ...
                    if last_line:
                        ...
                        if len(parts) >= 4:
                            ...
                            best_dice = float(parts[3])
                            print(f"检测到历史训练，从 Epoch {start_epoch} 继续训练")
        except Exception as e:
            print(f"读取历史文件失败，从头开始训练: {e}")

    # 加载最佳模型权重（如果存在）
    if best_model_path.exists() and start_epoch > 1:
        try:
            checkpoint = torch.load(best_model_path, map_location=device)
            model.load_state_dict(checkpoint["model_state_dict"])
            print(f"已加载最佳模型，当前最佳 Dice: {best_dice:.4f}")
        except Exception as e:
            print(f"加载模型失败，使用随机权重: {e}")

    # 根据是否续训选择写入模式
    if start_epoch > 1:
        f = history_path.open("a", encoding="utf-8", newline="")
        writer = csv.DictWriter(
            f,
            fieldnames=["epoch", "train_loss", "val_loss", "val_dice", "lr"],
        )
    else:
        f = history_path.open("w", encoding="utf-8", newline="")
        writer = csv.DictWriter(
            f,
            fieldnames=["epoch", "train_loss", "val_loss", "val_dice", "lr"],
        )
